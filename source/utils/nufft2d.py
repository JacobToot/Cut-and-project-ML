...
# ---------------------------------------------------------------------------
# Kaiser-Bessel gridding kernel (Beatty et al. 2005 parameterisation)
# ---------------------------------------------------------------------------
...
def _kb_kernel(dist: torch.Tensor, width: int, beta: float) -> torch.Tensor:
    ...
    half = width / 2.0
    z = 1.0 - (dist / half) ** 2
    z = z.clamp(min=0.0)                       # outside support -> 0 arg
...
def _kb_apodization(n_out: int, grid_os: int, width: int, beta: float,
    ...
    ker = _kb_kernel(p, width, beta)                         # centred kernel
    # zero-phase DFT: shift centre to index 0, transform, recentre frequencies
    apod = torch.fft.fftshift(
        torch.fft.fft(torch.fft.ifftshift(ker))).real        # (grid_os,)
    lo = grid_os // 2 - n_out // 2
    return apod[lo:lo + n_out]


# ---------------------------------------------------------------------------
# Backend 1: exact direct DFT (oracle + recommended default at this scale)
# ---------------------------------------------------------------------------
@torch.no_grad()
def structure_factor_direct(
    points: torch.Tensor,      # (B, N, 2)  real-space coords
    strengths: torch.Tensor,   # (B, N)     per-point weight (e.g. 0/1 mask)
    q_grid: torch.Tensor,      # (M, 2)     reciprocal-space sample points
    q_chunk: int = 4096,
) -> torch.Tensor:
...
# ---------------------------------------------------------------------------
# Backend 2: type-1 NUFFT (KB gridding + oversampled FFT + de-apodization)
# ---------------------------------------------------------------------------
@torch.no_grad()
def structure_factor_nufft(
    points_scaled: torch.Tensor,   # (B, N, 2) coords pre-scaled to [-pi, pi)
    strengths: torch.Tensor,       # (B, N)
    n_grid: int,                   # output grid is n_grid x n_grid
    sigma: float = 2.0,
    width: int = 6,
) -> torch.Tensor:
    ...
    # separable 2D weights and flat target indices
    ...
    # de-apodise: divide by separable KB spectral footprint
...
# ---------------------------------------------------------------------------
# Diffraction image producer (one interface, both backends)
# ---------------------------------------------------------------------------
@dataclass
...
class DiffractionImager(nn.Module):
    ...
    def __init__(self, cfg: DiffractionConfig | None = None, **kw):
        ...
        # 1D q axis in [-q_max, q_max); grid spacing dq = 2 q_max / g
    ...
    def forward(self, points: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
        ...
        if cfg.backend == "direct":
            ...
            # scale to integer-frequency convention and wrap to [-pi, pi)
            x = points * self.dq
...
# ---------------------------------------------------------------------------
# Self-test: NUFFT vs exact direct DFT, on diffuse + sharp-peaked inputs
# ---------------------------------------------------------------------------
...
def _rel_err(a: torch.Tensor, b: torch.Tensor) -> float:
    # compare image *shape* (scale-insensitive): normalise each to unit norm
    a = a / (a.norm() + 1e-12)
    b = b / (b.norm() + 1e-12)
...
def _self_test():
    ...
    cases = {
        ...
        "square_lattice": _square_lattice(18).unsqueeze(0),          # sharp peaks
    ...
    # timing at a more realistic size
